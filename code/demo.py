...
def classify_image(file_path):
    ...
    with torch.no_grad():
        ...
        # # Normalizzazione Min-Max per ottenere valori tra 0 e 1
        # min_vals = outputs.min(dim=-1, keepdim=True).values
        # max_vals = outputs.max(dim=-1, keepdim=True).values
        # normalized_outputs = (outputs - min_vals) / (max_vals - min_vals)


        # Estrazione dei top-10 valori normalizzati
