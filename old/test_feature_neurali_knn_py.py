...
def preprocess_image(image_path, target_size, augment):
    if augment:
        transform = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Lambda(lambda x: x + torch.randn_like(x) * 0.05),  # Aggiungi rumore gaussiano (std=0.05)
...
def extract_features(model, image_names, indir, target_size, device, augment=False):
    model = model.to(device)
    features = []
    for i, image_name in enumerate(image_names):
        print(f"Elaborazione immagine: {i + 1}/{len(image_names)}")
        image_path = os.path.join(indir, image_name)
        img = preprocess_image(image_path, target_size, augment).to(device)
        with torch.no_grad():
            feature = model(img)
...
# Training set
feat_tr = extract_features(model, image_names_tr, indir_tr, input_size, device, augment=True)

# Test set
feat_te = extract_features(model, image_names_te, indir_te, input_size, device, augment=False)
